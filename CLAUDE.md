...
| `भारत.सुरक्षित` | `https::surakshit_registry()` | **HTTPS/TLS client** (Phase 19, `src/https.rs`): `https_पाओ(url[, शीर्षक])` GET, `https_भेजो(url, शरीर[, शीर्षक])` POST → Dict {स्थिति, सामग्री}. Links Windows `winhttp.dll` for real TLS (the http-only `भारत.http` can't reach https). Unlocks LLM APIs (api.anthropic.com), cloud/payment services. https:// only; Windows now, WASM/other = catchable error. See `examples/llm_client.swami` for a Claude Messages-API wrapper (pairs with भारत.json) |
