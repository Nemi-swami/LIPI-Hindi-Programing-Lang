# LIPI — Language & Standard-Library Reference
...
| `src/yaml.rs` | `भारत.यामल` — YAML-subset reader (Phase 19 F7) |
| `src/xmlparse.rs` | `भारत.एक्सएमएल` — XML / well-formed-HTML reader → element tree (Phase 19 F8) |
| `src/argparse.rs` | `भारत.तर्कपार्स` — command-line argument parser → Dict (Phase 19 F9) |
...
| `भारत.बाह्य` | `ffi::bahya_registry()` | **C FFI — the "no limits" escape hatch** (Phase 19, `src/ffi.rs`): `बाह्य_पुस्तकालय(पथ)` load a DLL → handle, `बाह्य_बुलाओ(हैंडल, नाम, "<args>:<ret>", …)` resolve+call any C-ABI function, `बाह्य_बंद(हैंडल)` unload. Signature chars: `i` int32, `l` int64/ptr, `d` double, `s` C-string, `v` void. Pure Rust; cross-platform — LoadLibrary on Windows, dlopen on Linux/macOS (via a `plat` backend); WASM = catchable error. LIMITATION: one call is all-int/ptr/str OR all-double, no mixing (no libffi). Lets LIPI drive BLAS/OpenGL/OS/hardware SDKs → any domain. **Callbacks** (`src/cbthunk.rs`): `बाह्य_कॉलबैक(क्लोजर, "ll:i")` returns a C function pointer backed by a LIPI closure (for qsort comparators, window procs, driver callbacks) — int/ptr args only, ≤4, via a fixed trampoline pool; `बाह्य_कॉलबैक_मुक्त(पता)` frees a slot. Registered in the LVM Call handler (needs &mut self), NOT a plain native |
| `भारत.सुरक्षित` | `https::surakshit_registry()` | **HTTPS/TLS client** (Phase 19, `src/https.rs`): `https_पाओ(url[, शीर्षक])` GET, `https_भेजो(url, शरीर[, शीर्षक])` POST → Dict {स्थिति, सामग्री}. Links Windows `winhttp.dll` for real TLS (the http-only `भारत.http` can't reach https). Unlocks LLM APIs, cloud/payment services. https:// only; Windows now, WASM/other = catchable error. See `examples/llm_client.swami` for an LLM API wrapper (pairs with भारत.json) |
| `भारत.लॉग` | `lognaad::log_registry()` | **Logging** (Phase 19, `src/lognaad.rs`): `लॉग_स्तर(name)` sets threshold; `लॉग_डिबग`/`लॉग_सूचना`/`लॉग_चेतावनी`/`लॉग_त्रुटि(msg)` print to stderr if level ≥ threshold (default सूचना). Timestamped on native builds; WASM-safe |
| `भारत.टोमल` | `tomlparse::toml_registry()` | **Config** (Phase 19, `src/tomlparse.rs`): `toml_पढ़ो(पाठ)` TOML→nested Dict (tables, arrays, typed scalars); `ini_पढ़ो(पाठ)` INI→Dict of sections. Pure Rust |
| `भारत.यामल` | `yaml::yaml_registry()` | **Config** (Phase 19, `src/yaml.rs`): `यामल_पढ़ो(पाठ)` YAML-subset→Value (indent-based maps/lists/scalars). No anchors/flow/multi-doc |
| `भारत.एक्सएमएल` | `xmlparse::xml_registry()` | **Data** (Phase 19, `src/xmlparse.rs`): `xml_पढ़ो(पाठ)` → element Dict `{नाम, गुण, बच्चे, पाठ}`. Well-formed XML / XHTML-ish HTML |
| `भारत.तर्कपार्स` | `argparse::argparse_registry()` | **CLI** (Phase 19, `src/argparse.rs`): `तर्क_पार्स(सूची)` → Dict. `--key=value` sets a value, bare `--flag`/`-x` are booleans, rest → `_स्थिति` positionals |
