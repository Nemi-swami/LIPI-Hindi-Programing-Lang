...
use crate::ast::{unwrap_located, BinOp, Expr, Stmt};
...
fn codegen(params: &[String], body: &[Stmt]) -> Option<Vec<u8>> {
...
impl Drop for JitFn {
    fn drop(&mut self) { unsafe { mem::free(self.ptr); } }
}

/// Call JIT'd code at `ptr` with f64 args. UNSAFE: `ptr` must be a page this module
/// produced for exactly `args.len()` parameters.
...
/// JIT every qualifying top-level function in the program.
pub fn compile_program(stmts: &[Stmt]) -> HashMap<String, JitFn> {
    ...
    for s in stmts {
        if let Stmt::Vidhi { name, params, body, vararg, decorators, is_static, .. } = unwrap_located(s) {
            ...
            if let Some(code) = codegen(&pnames, body) {
